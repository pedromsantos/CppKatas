...
public:
  ...
  InstructionToCommand(const char instruction) const {
...
public:
  ...
  void ParseGridSizeInstruction(std::unique_ptr<Commands> &commands,
                                const std::string &line) const;

  void ParsePositionInstruction(std::unique_ptr<Commands> &commands,
                                const std::string &line) const;

  void ParseDirectionInstruction(std::unique_ptr<Commands> &commands,
                                 const std::string &line) const;

  void ParseMovementsInstruction(const std::unique_ptr<Commands> &commands,
                                 const std::string &line) const;
};
...
void InstructionParser::InstructionParserImpl::ParseGridSizeInstruction(
    std::unique_ptr<Commands> &commands, const std::string &line) const {
...
void InstructionParser::InstructionParserImpl::ParsePositionInstruction(
    std::unique_ptr<Commands> &commands, const std::string &line) const {
  auto tokens = Split(line, ' ');
...
void InstructionParser::InstructionParserImpl::ParseDirectionInstruction(
    std::unique_ptr<Commands> &commands, const std::string &line) const {
  auto tokens = Split(line, ' ');
  auto rawDirection = tokens.back();
  auto direction =
      instructionsToDirections_.InstructionToDirection(rawDirection);
  commands->AddCommand(std::make_unique<DirectionCommand>(direction));
}

void InstructionParser::InstructionParserImpl::ParseMovementsInstruction(
    const std::unique_ptr<Commands> &commands, const std::string &line) const {
...
std::string Position::ToString() const { return pimpl->ToString(); }
...
public:
  Direction Turn(const Direction direction) const { return map.at(direction); }
};
...
struct Rover::RoverImpl {
  ...
  void TurnLeft() { direction_ = turnLeft_.Turn(direction_); }

  void TurnRight() { direction_ = turnRight_.Turn(direction_); }

  void Move() { position_ = position_->Move(direction_); }
...
void Rover::TurnLeft() { pimpl->TurnLeft(); }

void Rover::TurnRight() { pimpl->TurnRight(); }

void Rover::Move() { pimpl->Move(); }
